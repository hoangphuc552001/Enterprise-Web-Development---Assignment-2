...
const ActorCard = ({ actor }: ActorCardProps) => {
  const { isFavourite, toggleFavourite } = useFavouriteActors();
  const favourite = isFavourite(actor.id);

  const handleToggle = (e: React.MouseEvent) => {
    e.preventDefault();
    toggleFavourite(actor.id);
  };
  ...
  return (
    <Card sx={{ position: "relative" }}>
      ...
      >
        <Avatar
          src={
            actor.profile_path ? `${PROFILE_BASE_URL}${actor.profile_path}` : ""
          }
          alt={actor.name ?? ""}
          ...
          <Stack spacing={1}>
            ...
            {actor.known_for && actor.known_for.length > 0 && (
              <Typography variant="body2" color="text.secondary">
                Notable works:{" "}
                {actor.known_for
                  .map((w) => w.title || w.original_title)
                  .filter(Boolean)
                  .join(", ")}
              </Typography>
            )}
